...
export function renderModals(state: AppState): string {
  return `${renderEntryModal(state)}${renderTxnModal(state.txnKind)}${renderTaskModal(state)}${renderTaskDetailModal(state)}${renderGameModal(state)}${renderGameDetailModal(state)}
