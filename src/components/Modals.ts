...
import { renderTaskDetailModal, renderTaskModal } from '../pages/WorkPage';
import { renderGameDetailModal, renderGameModal } from '../pages/GamesPage';

export function renderModals(state: AppState): string {
  return `${renderEntryModal(state)}${renderTxnModal(state.txnType)}${renderTaskModal(state)}${renderTaskDetailModal(state)}${renderGameModal(state)}${renderGameDetailModal(state)}
